...
On Modal, a function or class method can be exposed to web traffic by decorating it with [`@app.asgi_app()`](https://modal.com/docs/sdk/py/latest/modal.asgi_app#modalasgi_app) and returning a FastAPI app. You're then free to configure the FastAPI server however you like, including adding middleware, serving static files, and running websockets.
