...
extracted answer changes have their judge verdicts dropped so they can be
re-graded.
...
def reextract_record(record: dict, *, keep_judges: bool) -> dict:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Model label under models/ (default: af-next-think).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    pred_path = args.pack / "models" / args.model / "predictions.jsonl"
    ...
    for record in records:
        ...
        stats = reextract_record(record, keep_judges=args.keep_judges)
        ...
        if args.examples and len(examples) < args.examples:
            for shot, old in zip(record.get("shots") or [], before):
                ...
                if new == old:
                    continue
                examples.append(
                    (qid, int(shot.get("shot_index", 0)), old, new)
                )
                if len(examples) >= args.examples:
                    break

    print(
        f"{args.model}: {len(records)} records, {totals['shots']} shots, "
    ...
    for qid, shot_index, old, new in examples:
        print(f"\n--- {qid} shot {shot_index} ---")
        print(f"OLD: {old}")
        print(f"NEW: {new}")
